...
class DWRTester(test_utils.Tester):
    ...
    def define_model(self):
        ...
        if opts.pred_voxels:
             self.voxel_decoder = self.model.code_predictor.shape_predictor.decoder
    ...
    def save_current_visuals(self, image_id):
        imgs_dir=osp.join(self.opts.results_quality_dir, '{}'.format(image_id))
    ...
    def test_draw(self):
        ...
        if index_filename is not None:
            with open(index_filename) as f:
                for line in f:
                    line=line.strip()
                    image_ids.append('_'.join(line.split('_')))

        rng = np.random.RandomState(1)
        indices = rng.choice(len(self.dataloader), 50)
        dump_to_file = False
        if len(image_ids) == 0:
            dump_to_file = True
            dump_file_fd = open(osp.join(base_dir,'random_imgs.txt'),'w') 
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            if self.invalid_batch:
                continue
            # pdb.set_trace()
            image_id=batch['image_name'][0]
            example_id='{}'.format(image_id)
            if example_id in image_ids or (i in indices and len(image_ids) == 0):
                self.predict()
                bench_image_stats = self.evaluate()
                self.save_current_visuals(image_id)
