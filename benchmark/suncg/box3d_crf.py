...
"""
# Sample usage:
# (shape_ft) : python -m relative3d.benchmark.suncg.box3d_crf_save_predictions --num_train_epoch=8 --name=box3d_base_vanilla --classify_rot --pred_voxels=False --use_context --save_visuals --visuals_freq=50 --eval_set=val --pred_relative=False --suncg_dir=/nvme-scratch/nileshk/suncg/ --preload_stats=False --crf_optimize=True --results_name=box3d_base_vanilla_trans_scale_crf_small --do_updates=True --max_eval_iter=20
...
flags.DEFINE_boolean('crf_optimize', False, 'Optimize using CRF potentials')
flags.DEFINE_boolean('pred_mean', False, 'use mean to compute error')
...
class DWRTester(test_utils.Tester):
    ...
    def define_mean_unary_potentials(self,):

        opts = self.opts
        unary_params = np.load(osp.join(opts.cache_dir, 'crf_models', 'unary_params.npy')).item()
        self.mean_trans = unary_params['trans']
        self.mean_scale = unary_params['scale']
        self.mean_quat = unary_params['quat_quant']
        return

    def define_crf_potentials(self,):
        opts = self.opts
        ## Load all the mixture weights for scale and translation.
        
        relative_gmm_params = np.load(osp.join(opts.cache_dir, 'crf_models', 'relative_gmm_params_10.npy')).item()

        keys = ['relative_trans', 'relative_scale', 'relative_direction_quant']
        trans_variances = [v['covariance'] for v in relative_gmm_params['relative_trans'].values()]
        trans_variances = np.stack(trans_variances)

        relative_trans_potential_functions = {}
        relative_scale_potential_functions = {}
        relative_dir_potential_functions = {}




        for clsid in relative_gmm_params[keys[0]].keys():
            gmm_params = relative_gmm_params[keys[0]][clsid]
            if gmm_params is None:
                relative_trans_potential_functions[clsid] = crf_utils.CRFZeroPotential()
            else:
                binary_potential = crf_utils.convert_params_to_binary_potential(gmm_params['weights'], gmm_params['means'], gmm_params['covariance'])
                relative_trans_potential_functions[clsid] = binary_potential
    
        for clsid in relative_gmm_params[keys[1]].keys():
            gmm_params = relative_gmm_params[keys[1]][clsid]
            if gmm_params is None:
                relative_scale_potential_functions[clsid] = crf_utils.CRFZeroPotential()
            else:
                binary_potential = crf_utils.convert_params_to_binary_potential(gmm_params['weights'], gmm_params['means'], gmm_params['covariance'])
                relative_scale_potential_functions[clsid] = binary_potential


        for clsid in relative_gmm_params[keys[2]].keys():
            dir_quat = relative_gmm_params[keys[2]][clsid]
            relative_dir_potential_functions[clsid] = torch.from_numpy(dir_quat)


        self.relative_dir_potential_functions = relative_dir_potential_functions
        self.relative_trans_potential_functions =  relative_trans_potential_functions
        self.relative_scale_potential_functions =  relative_scale_potential_functions
    ...
    def init_dataset(self):
        ...
        # house_names = [l.strip() for l in open('small.txt')]
        # self.split = suncg_parse.get_split(split_dir, house_names=house_names)
        ...
        # self.split[opts.eval_set] = house_names
        ...
        self.define_crf_potentials()
        self.define_mean_unary_potentials()
        return
    ...
    def get_current_visuals(self):
        ...
        codes_pred_eval_copy = {}
        codes_pred_eval_copy['trans'] = self.codes_pred_eval['trans']
        codes_pred_eval_copy['scale'] = self.codes_gt['scale']
        codes_pred_eval_copy['quat'] = self.codes_gt['quat']
        codes_pred_eval_copy['shape'] = self.codes_gt['shape']
    ...
    def predict(self):
        ...
        if opts.crf_optimize :

            unaries = self.codes_pred_eval['trans'].data.cpu()
            object_classes = self.object_classes.data.cpu()
            class_pair_potentials = self.relative_trans_potential_functions
            crfmodel = crf_utils.CRFModelTrans(unaries.clone(), object_classes, class_pair_potentials, pairwise_wt=0.1, unary_wt=100, use_max_pool=False)
            # crfmodel = crf_utils.CRFModelTransDebug(unaries.clone(), object_classes, self.relative_gt['relative_trans'].data.cpu(), pairwise_wt=0.1, unary_wt=100.0)
            crf_opts = lambda x: 0
            crf_opts.learning_rate = 0.1
            crf_opts.delta_reg = 1E-5
            crfoptim = crf_utils.CRFOptimizer(crfmodel, crf_opts, verbose=False, max_iter=100)
            # pdb.set_trace()
            crfoptim.optimize()

            if opts.do_updates:
                new_trans = []
                self.update_norm = torch.norm(crfmodel.parameters().data - self.codes_pred_eval['trans'].data.cpu(), dim=1).numpy().tolist()
                for ox, un in enumerate(self.update_norm):
                    if un < 3:
                        new_trans.append(crfmodel.parameters().data[ox])
                    else:
                        new_trans.append(unaries[ox])
                new_trans = torch.stack(new_trans)
                self.codes_pred_eval['trans'] = new_trans.data.cuda()

        self.update_norm = torch.norm(self.codes_pred_eval['scale'].data.cpu()*0, dim=1).numpy().tolist()

        if opts.crf_optimize:
            unaries = self.codes_pred_eval['scale'].data.cpu().log()
            object_classes = self.object_classes.data.cpu()
            class_pair_potentials = self.relative_scale_potential_functions
            crfmodel = crf_utils.CRFModelTrans(unaries.clone(), object_classes, class_pair_potentials, pairwise_wt=.1 , unary_wt= 100.0, use_max_pool=False)
            crf_opts = lambda x: 0
            crf_opts.learning_rate = 0.001
            crf_opts.delta_reg = 1E-5
            crfoptim = crf_utils.CRFOptimizer(crfmodel, crf_opts, verbose=False, )
            crfoptim.optimize()

            if opts.do_updates:
                #self.codes_pred_eval['scale'] = crfmodel.parameters().data.cuda().exp()
                new_scale = []
                self.update_norm = torch.norm(crfmodel.parameters().data - self.codes_pred_eval['scale'].data.cpu(), dim=1).numpy().tolist()
                for ox, un in enumerate(self.update_norm):
                    if un < 3:
                        new_scale.append(crfmodel.parameters().data[ox])
                    else:
                        new_scale.append(unaries[ox])
                new_scale = torch.stack(new_scale)
                self.codes_pred_eval['scale'] = new_scale.data.cuda().exp()
        ...
        if opts.crf_optimize:
            class_pair_relative_probs = self.relative_dir_potential_functions
            object_classes = self.object_classes.data.cpu()
            crf_relative_model = crf_utils.RelativeRotationOptimizer(self.quat_medoids, self.direction_medoids, opts.lambda_weight, class_pair_relative_probs)
            relative_dir_probs = torch.log(crf_relative_model.get_relative_dir(object_classes)+1E-10).data.numpy()
            
            absolute_location = self.codes_pred_eval['trans'].data.cpu().numpy()
            absolute_log_probabilites = self.codes_pred_eval['quat'].data.cpu().numpy()

            new_quat_probs = crf_relative_model.forward(absolute_location, absolute_log_probabilites, relative_dir_probs)
            self.codes_pred_eval['quat'] = Variable(new_quat_probs)
        self.entropy_after_optim = (-1 * self.codes_pred_eval['quat']  * (self.codes_pred_eval['quat'] + 1E-10).log()).sum(1).data.cpu().numpy()


        if opts.pred_mean:
            mean_obj_trans = [self.mean_trans['cls_{}'.format(clsid.item())]['mean'] for clsid in self.object_classes.data]
            mean_obj_trans = torch.from_numpy(np.stack(mean_obj_trans)).cuda()
            self.codes_pred_eval['trans'] = mean_obj_trans

            mean_obj_scale = [self.mean_scale['cls_{}'.format(clsid.item())]['median'] for clsid in self.object_classes.data]
            mean_obj_scale = torch.from_numpy(np.stack(mean_obj_scale)).cuda()
            self.codes_pred_eval['scale'] = mean_obj_scale

            likli_obj_rotation = [self.mean_quat['cls_{}'.format(clsid.item())] for clsid in self.object_classes.data]
            likli_obj_rotation = np.stack(likli_obj_rotation)
            self.codes_pred_eval['quat']  = Variable(torch.from_numpy(likli_obj_rotation).cuda())
